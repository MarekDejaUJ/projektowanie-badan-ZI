...
run <- function() {
  ...
  probe <- paste0("stopifnot(Sys.getuid()!=0L, all(Sys.getenv(c('GH_TOKEN','GITHUB_TOKEN','GITHUB_PAT'))==''), ",
    "!file.exists('/var/run/docker.sock'), identical(list.files('/sys/class/net'),'lo')); ",
    "s<-readLines('/proc/self/status'); stopifnot(any(grepl('^NoNewPrivs:[[:space:]]+1$',s)), ",
