# Gotowy skrypt ćwiczenia C10: odczyt wariantu zapisanego na C09, obie analizy
# projektu, tabele do akapitów, wykresy i karta odtworzenia. Bloki Rmd
...
# ---- C10: odczyt wariantu zapisanego na C09 ----
...
przygotuj_wariant_projektu <- function(scenariusz=NULL,id=identyfikator_materialu()) {
  # Własna praca odczytuje kopię wariantu Z09; podgląd DEMO_C09 i przykłady demo001 generują dane.
  pakiet <- if(missing(id) && !identical(id,'DEMO_C09'))
    badaniaZI::wariant_projektu('Z10',scenariusz,id_studenta=id) else
...
# ---- C10: demonstracje LEARN ----
symbole_do_odczytu <- function() {
  wiersze <- c('| $N$, $n_1$, $n_2$ | ważne osoby lub pary; liczebności grup | $N = 147$ (indeks); $n = 74$ i $n = 73$ |',
    '| $\\widehat\\Delta$ | oszacowanie różnicy w jednostce pomiaru | $-0{,}438$ punktu indeksu |',
    '| $SD$, $SE$ | rozrzut osób; niepewność estymaty | $SD = 0{,}704$ i $0{,}687$; $SE = 0{,}115$ |',
    '| $CI$ | przedział dla parametru przy założeniach metody | $[-0{,}665; -0{,}212]$ (Welch) |',
    '| $t$, $\\chi^2$ | statystyka właściwego testu | $t(144{,}98) = -3{,}822$; $\\chi^2(1) = 1{,}466$ |',
    '| $p$, $p_{\\mathrm{perm}}$, $p_{\\mathrm{MC}}$ | skrajność wyniku przy modelu zerowym | $p = 0{,}0002$; $p_{\\mathrm{perm}} = 0{,}001$; $p_{\\mathrm{MC}} = 0{,}297$ |',
    '| $B$, $k$ | liczba losowań; wyniki co najmniej tak skrajne | $B = 1999$, $k = 1$ (tasowanie etykiet) |')
  wydruk_zi(markdown=list(c('**Symbole wyniku i przykład z S02**','',
    '| Symbol | Znaczenie | Przykład z S02 |',
    '|:----------------|:------------------------------|:------------------------------|',wiersze)))
}
precyzja_do_raportu <- function() {
  porownanie <- data.frame(Wariant=c('Szeroki','Precyzyjnie mały','Wyraźna różnica'),
    Estymata=c(.2,.2,.8),dol=c(-.5,.1,.5),gora=c(.9,.3,1.1))
  tab <- data.frame(Wariant=porownanie$Wariant,'Estymata [pkt]'=liczba(porownanie$Estymata,1L),
    'Dolna granica'=liczba(porownanie$dol,1L),'Górna granica'=liczba(porownanie$gora,1L),check.names=FALSE)
  wykres <- badaniaZI::wykres_przedzialy(porownanie$Wariant,porownanie$Estymata,porownanie$dol,
    porownanie$gora,odniesienie=0,os='Hipotetyczna różnica [punkty indeksu]',
    tytul='Estymata i precyzja: trzy hipotetyczne oszacowania')
  wydruk_zi(tabele=list('Trzy hipotetyczne oszacowania różnicy'=tab),wykresy=list(wykres),digits=1L)
}
...
droga_raportu <- function() {
  badaniaZI::diagram_etapow(c('Zadanie Z09 (C09)','Zadanie Z10 (C10)','przygotuj_raport()',
      'raport.Rmd','oddaj_projekt()'),
    c('Akapity S01–S05: problem, pytania, próba i pomiar, przygotowanie danych, pierwszy opis',
...
analizy_demo <- analizy_wariantu(wariant_demo)
wariant_korelacji <- przygotuj_wariant_projektu('S05',id='demo001')
analizy_korelacji <- analizy_wariantu(wariant_korelacji)
