...
porownanie_ci <- data.frame(wariant=c('Szeroki','Precyzyjnie mały','Wyraźna różnica'),
  efekt=c(.2,.2,.8),dol=c(-.5,.1,.5),gora=c(.9,.3,1.1))
precyzja_do_raportu <- function() {
  plot <- ggplot2::ggplot(porownanie_ci,ggplot2::aes(efekt,wariant))+
    ggplot2::geom_segment(ggplot2::aes(x=dol,xend=gora,yend=wariant),
      linewidth=.8,colour='#0072B2')+ggplot2::geom_point(size=2,colour='#D55E00')+
    ggplot2::geom_vline(xintercept=0,linetype=2)+
    ggplot2::labs(x='Hipotetyczna różnica [punkty indeksu]',y=NULL,
      title='Efekt i precyzja to odrębne informacje')+badaniaZI::theme_zi()
  wydruk_zi(tabele=list('Trzy hipotetyczne oszacowania — nie wyniki S02'=porownanie_ci),
    wykresy=list(plot))
}
