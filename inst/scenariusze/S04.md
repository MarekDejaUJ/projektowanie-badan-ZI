...
Biblioteka publiczna porównuje doświadczenia odbiorców informacji przez WWW i media społecznościowe. Chce odróżnić łatwość znalezienia komunikatu od deklarowanego udziału w wydarzeniu, na który wpływają też termin, zainteresowania i dostępność miejsca.
...
Grupy: WWW / media społecznościowe. WWW i media społecznościowe oznaczają jeden deklarowany główny kanał szukania informacji o wydarzeniach. To nie są wszystkie zaznaczenia pytania wielokrotnego; dana osoba może korzystać także z innych kanałów.

Obserwacyjne badanie przekrojowe; grupy nie są randomizowane.

Indeks: Ocena łatwości dotarcia do informacji o wydarzeniach. To wynik z sześciu deklarowanych ocen, nie test wszystkich kompetencji ani niezależny audyt usługi.

Pierwsze pytanie: jaka jest różnica średnich indeksu w porównaniu „WWW minus media społecznościowe”? Estymandą jest różnica średnich w populacjach określonych w planie; estymatę obliczamy z dostępnej próby. Jednostka to punkt indeksu. H0 zakłada różnicę równą zero.

Druga relacja: główny kanał × deklarowany udział w wydarzeniu.
Drugie pytanie dotyczy różnicy prawdopodobieństw kodu 1: „WWW minus media społecznościowe”. H0 zakłada równe prawdopodobieństwa. Zanim nazwiesz odsetek sukcesem, przeczytaj definicję kodów poniżej.
...
Zadanie: Znajdź datę, miejsce i warunki udziału w wskazanym wydarzeniu, korzystając z przygotowanego komunikatu.
...
Pole `powodzenie` zachowuje wspólną nazwę techniczną. Treść pytania: „Czy uczestniczyłeś w wydarzeniu?”

Źródło wartości: odpowiedź ankietowa — deklaracja respondenta, nie obserwacja wykonania.

Kod 1: zadeklarowano udział w co najmniej jednym wydarzeniu tej biblioteki w ostatnich czterech tygodniach.
Kod 0: zadeklarowano brak takiego udziału.
NA oznacza nieznany wynik lub brak odpowiedzi, a nie kod 0.

Udział jest wspomnieniem respondenta, a nie obserwowanym sukcesem zadania ani dowodem działania wybranego kanału.
