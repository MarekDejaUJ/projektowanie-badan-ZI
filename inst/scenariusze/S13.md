# S13 — Instytucjonalne FAQ: adekwatność odpowiedzi

Wersja opisu: 1.1.0. Dane do analizy są syntetyczne.

Instytucja chce poprawić FAQ tak, aby odpowiadało na sprawy odbiorców. Porównuje ocenę adekwatności i deklarację rozwiązania problemu; brak kontaktu z obsługą sam w sobie nie oznacza rozwiązania sprawy.
...
Grupy: pierwszy raz / powracający. Pierwszy raz i powracający oznaczają pierwszy lub kolejny kontakt z tym FAQ. To nie jest pomiar tej samej osoby przed i po zmianie strony. Powracający mogą mieć bardziej złożone sprawy.

Obserwacyjne badanie przekrojowe; grupy nie są randomizowane.

Indeks: Ocena adekwatności odpowiedzi FAQ. To wynik z sześciu deklarowanych ocen, nie test wszystkich kompetencji ani niezależny audyt usługi.

Pierwsze pytanie: jaka jest różnica średnich indeksu w porównaniu „pierwszy raz minus powracający”? Estymandą jest różnica średnich w populacjach określonych w planie; estymatę obliczamy z dostępnej próby. Jednostka to punkt indeksu. H0 zakłada różnicę równą zero.

Druga relacja: grupa × deklarowane rozwiązanie problemu.
Drugie pytanie dotyczy różnicy prawdopodobieństw kodu 1: „pierwszy raz minus powracający”. H0 zakłada równe prawdopodobieństwa. Zanim nazwiesz odsetek sukcesem, przeczytaj definicję kodów poniżej.
...
Zadanie: Odszukaj odpowiedź na jedną opisaną sprawę w FAQ i zapisz proponowany sposób jej rozwiązania. Następnie oceń, czy znaleziona odpowiedź pozwoliła rozwiązać tę sprawę.
...
1. FAQ zawiera pytanie odpowiadające mojej sprawie.
2. Odpowiedź podaje potrzebne kroki działania.
3. Odpowiedzi FAQ są zbyt ogólne, aby mi pomóc.
4. Odpowiedź wskazuje, kiedy zasada nie ma zastosowania.
5. Linki w odpowiedzi pozwalają sprawdzić szczegóły.
6. FAQ wyjaśnia, gdzie uzyskać dodatkową pomoc.

Pole `powodzenie` zachowuje wspólną nazwę techniczną. Treść pytania: „Czy FAQ pozwoliło rozwiązać problem?”

Źródło wartości: odpowiedź ankietowa — deklaracja respondenta, nie obserwacja wykonania.

Kod 1: zadeklarowano, że FAQ pozwoliło rozwiązać opisaną sprawę.
Kod 0: zadeklarowano, że FAQ nie pozwoliło jej rozwiązać.
NA oznacza nieznany wynik lub brak odpowiedzi, a nie kod 0.

To ocena respondenta, nie niezależna weryfikacja poprawności rozwiązania. Wspólne źródło obu deklaracji może ograniczać interpretację ich zgodności.
