...
Muzeum chce rozpoznać, jak odbiorcy o różnych celach korzystają z opisów kolekcji. Pytanie o kompletność jest oceną z perspektywy potrzeby użytkownika, nie automatycznym audytem zgodności rekordów ze standardem metadanych.
...
Grupy: edukacyjni / hobbystyczni. Edukacyjni i hobbystyczni oznaczają deklarowany główny cel bieżącej wizyty. Osoba może mieć oba zainteresowania, lecz wskazuje cel dominujący dla tej sesji; nie jest liczona w obu grupach.

Obserwacyjne badanie przekrojowe; grupy nie są randomizowane.

Indeks: Postrzegana kompletność metadanych. To wynik z sześciu deklarowanych ocen, nie test wszystkich kompetencji ani niezależny audyt usługi.

Pierwsze pytanie: jaka jest różnica średnich indeksu w porównaniu „edukacyjni minus hobbystyczni”? Estymandą jest różnica średnich w populacjach określonych w planie; estymatę obliczamy z dostępnej próby. Jednostka to punkt indeksu. H0 zakłada różnicę równą zero.

Druga relacja: grupa × znalezienie potrzebnego obiektu.
Drugie pytanie dotyczy różnicy prawdopodobieństw kodu 1: „edukacyjni minus hobbystyczni”. H0 zakłada równe prawdopodobieństwa. Zanim nazwiesz odsetek sukcesem, przeczytaj definicję kodów poniżej.
...
Zadanie: Znajdź obiekt odpowiadający podanemu twórcy, okresowi i typowi oraz wskaż jego rekord.
...
Pole `powodzenie` zachowuje wspólną nazwę techniczną. Treść pytania: „Czy udało się znaleźć potrzebny obiekt?”

Źródło wartości: zapis obserwatora według podanego kryterium.

Kod 1: wybrano rekord spełniający wszystkie podane warunki.
Kod 0: zakończono próbę bez właściwego rekordu.
NA oznacza nieznany wynik lub brak odpowiedzi, a nie kod 0.

Znalezienie obiektu nie dowodzi zrozumienia jego kontekstu; subiektywnie kompletne metadane nie muszą być kompletne według normy katalogowania.
