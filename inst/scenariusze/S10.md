# S10 — Baza danych badawczych: gotowość do ponownego użycia danych

Wersja opisu: 1.1.0. Dane do analizy są syntetyczne.

Repozytorium danych rozważa poprawę opisów zmiennych i warunków użycia zbioru. Chce porównać ocenę dokumentacji oraz deklarowaną gotowość ponownego użycia, oddzielając zamiar od późniejszego pobrania, analizy i cytowania danych.
...
Grupy: z doświadczeniem / bez doświadczenia. Z doświadczeniem: wcześniejsze samodzielne wykorzystanie cudzego zbioru danych w analizie; bez doświadczenia: brak takiej praktyki. Samo pobranie pliku nie wystarcza do przypisania doświadczenia.

Obserwacyjne badanie przekrojowe; grupy nie są randomizowane.

Indeks: Ocena użyteczności dokumentacji danych. To wynik z sześciu deklarowanych ocen, nie test wszystkich kompetencji ani niezależny audyt usługi.
...
Druga relacja: grupa × deklaracja ponownego użycia.
Drugie pytanie dotyczy różnicy prawdopodobieństw kodu 1: „z doświadczeniem minus bez doświadczenia”. H0 zakłada równe prawdopodobieństwa. Zanim nazwiesz odsetek sukcesem, przeczytaj definicję kodów poniżej.
...
Zadanie: W dokumentacji wskazanego zbioru znajdź znaczenie zmiennej, jej jednostkę i informację o warunkach użycia.
...
1. Dokumentacja opisuje znaczenie zmiennych.
2. Wiem, jakie jednostki zastosowano w zbiorze.
3. Nie potrafię odczytać kodów brakujących danych.
4. Dokumentacja wyjaśnia sposób zbierania danych.
5. Warunki ponownego użycia są zrozumiałe.
6. Dokumentacja wystarcza do rozpoczęcia analizy.

Pole `powodzenie` zachowuje wspólną nazwę techniczną. Treść pytania: „Czy deklarujesz ponowne użycie zbioru danych?”

Źródło wartości: odpowiedź ankietowa — deklaracja respondenta, nie obserwacja wykonania.

Kod 1: po zapoznaniu się z opisanym zbiorem zadeklarowano zamiar jego ponownego użycia.
Kod 0: nie zadeklarowano takiego zamiaru.
NA oznacza nieznany wynik lub brak odpowiedzi, a nie kod 0.

Zamiar nie jest obserwowanym użyciem ani obietnicą, że zbiór rzeczywiście nadaje się do przyszłej analizy. Brak odpowiedzi nie oznacza odmowy.
