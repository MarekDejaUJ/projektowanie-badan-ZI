...
Organizacja chce ograniczyć korzystanie z nieaktualnych procedur. Zestawia deklarowane zaufanie do bazy wiedzy z czasem odnalezienia instrukcji; zaufanie nie zastępuje kontroli wersji dokumentu.
...
Grupy: krótki staż / długi staż. Krótki staż: mniej niż rok pracy w organizacji; długi staż: co najmniej rok. Próg jest przykładem planistycznym, nie cechą naturalną. W planie uwzględnij różne role i uprawnienia pracowników.

Obserwacyjne badanie przekrojowe; grupy nie są randomizowane.

Indeks: Deklarowane zaufanie do aktualności instrukcji. To wynik z sześciu deklarowanych ocen, nie test wszystkich kompetencji ani niezależny audyt usługi.

Pierwsze pytanie: jaka jest różnica średnich indeksu w porównaniu „krótki staż minus długi staż”? Estymandą jest różnica średnich w populacjach określonych w planie; estymatę obliczamy z dostępnej próby. Jednostka to punkt indeksu. H0 zakłada różnicę równą zero.

Druga relacja: czas wyszukiwania a indeks.
...
Zadanie: Znajdź instrukcję dla opisanej czynności i sprawdź oznaczenie aktualnej wersji.
...
Pole `powodzenie` zachowuje wspólną nazwę techniczną. Treść pytania: „Czy udało się znaleźć aktualną instrukcję?”

Źródło wartości: zapis obserwatora według podanego kryterium.

Kod 1: wskazano obowiązującą wersję zgodną z kluczem zadania.
Kod 0: zakończono próbę bez wskazania obowiązującej wersji.
NA oznacza nieznany wynik lub brak odpowiedzi, a nie kod 0.

Zaufanie do aktualności jest deklaracją. Długi czas może oznaczać staranną kontrolę wersji, a nie niesprawność.
...
Pierwsza analiza porównuje indeks: krótki staż minus długi staż. Odczytaj N i średnie grup, różnicę, SE, t Welcha, df i p. Podaj 95% CI różnicy Welcha i bootstrapu osób w grupach oraz Hedgesa g jako efekt bez jednostki. Przedział różnicy w punktach nie jest przedziałem dla g. Porównaj z p po tasowaniu etykiet przy wymienialności pod modelem zerowym; podaj B i wyjaśnij ocenę skrajności.
