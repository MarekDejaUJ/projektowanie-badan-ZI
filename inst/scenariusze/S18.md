# S18 — Informacja biblioteczna: komunikowanie ograniczeń dostępu

Wersja opisu: 1.1.0. Dane do analizy są syntetyczne.

Biblioteka chce odróżnić niejasny komunikat od ograniczenia wynikającego z uprawnień lub połączenia. Porównuje ocenę jasności z wystąpieniem nieudanego dostępu; nawet zrozumiałe zasady nie gwarantują, że każda osoba ma uprawnienia.
...
Grupy: lokalni / zdalni. Lokalni i zdalni oznaczają miejsce lub tryb połączenia podczas jednej próby dostępu. Dostęp zdalny nie jest przydzieloną interwencją; grupy mogą różnić się uprawnieniami, konfiguracją i wcześniejszym doświadczeniem.

Obserwacyjne badanie przekrojowe; grupy nie są randomizowane.

Indeks: Ocena jasności zasad dostępu. To wynik z sześciu deklarowanych ocen, nie test wszystkich kompetencji ani niezależny audyt usługi.

Pierwsze pytanie: jaka jest różnica średnich indeksu w porównaniu „lokalni minus zdalni”? Estymandą jest różnica średnich w populacjach określonych w planie; estymatę obliczamy z dostępnej próby. Jednostka to punkt indeksu. H0 zakłada różnicę równą zero.

Druga relacja: grupa × nieudana próba dostępu.
Drugie pytanie dotyczy różnicy prawdopodobieństw kodu 1: „lokalni minus zdalni”. H0 zakłada równe prawdopodobieństwa. Zanim nazwiesz odsetek sukcesem, przeczytaj definicję kodów poniżej.
...
Zadanie: Wykonaj jedną wskazaną próbę dostępu do zasobu. Zapis obserwatora obejmuje wynik tej próby, a nie dowolny wcześniejszy błąd podczas całej sesji.
...
1. Rozumiem, kto może korzystać z zasobu.
2. Informacja podaje wymagany sposób logowania.
3. Nie wiem, dlaczego dostęp może zostać odrzucony.
4. Rozumiem różnicę między dostępem lokalnym a zdalnym.
5. Komunikat wskazuje, jak rozwiązać problem dostępu.
6. Zasady jasno opisują ograniczenia wykorzystania zasobu.

Pole `powodzenie` zachowuje wspólną nazwę techniczną. Treść pytania: „Czy wystąpiła nieudana próba dostępu?”

Źródło wartości: zapis obserwatora według podanego kryterium.

Kod 1: wskazana, zarejestrowana próba zakończyła się nieuzyskaniem dostępu.
Kod 0: wskazana próba zakończyła się uzyskaniem dostępu.
NA oznacza nieznany wynik lub brak odpowiedzi, a nie kod 0.

Kod 1 oznacza niepowodzenie, a 0 uzyskanie dostępu. Utrata zapisu nie jest ani niepowodzeniem, ani sukcesem; wymaga NA. Wynik nie ustala przyczyny odmowy.
