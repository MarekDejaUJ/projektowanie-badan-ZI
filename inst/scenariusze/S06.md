# S06 — Newsletter instytucji: informacyjne dopasowanie treści

Wersja opisu: 1.1.0. Dane do analizy są syntetyczne.

Instytucja rozważa zmianę układu i doboru treści newslettera. Chce oddzielić ocenę jego przydatności od deklaracji wykorzystania informacji; otwarcie wiadomości lub kliknięcie linku nie przesądza o użyciu treści.
...
Grupy: regularni / okazjonalni. Regularni i okazjonalni oznaczają deklarowany zwyczaj czytania newslettera w ostatnim semestrze. Kryterium nie jest otwarciem jednej wiadomości. Zaproszenie wyłącznie w newsletterze może pominąć osoby, które przestały go czytać.

Obserwacyjne badanie przekrojowe; grupy nie są randomizowane.

Indeks: Ocena przydatności newslettera. To wynik z sześciu deklarowanych ocen, nie test wszystkich kompetencji ani niezależny audyt usługi.

Pierwsze pytanie: jaka jest różnica średnich indeksu w porównaniu „regularni minus okazjonalni”? Estymandą jest różnica średnich w populacjach określonych w planie; estymatę obliczamy z dostępnej próby. Jednostka to punkt indeksu. H0 zakłada różnicę równą zero.

Druga relacja: grupa × deklarowane wykorzystanie informacji.
Drugie pytanie dotyczy różnicy prawdopodobieństw kodu 1: „regularni minus okazjonalni”. H0 zakłada równe prawdopodobieństwa. Zanim nazwiesz odsetek sukcesem, przeczytaj definicję kodów poniżej.
...
Zadanie: W jednym wydaniu newslettera odszukaj wskazaną informację i link do jej szczegółów.
...
1. Newsletter odpowiada na moje potrzeby informacyjne.
2. Tytuły wiadomości pomagają wybrać treść.
3. Większość treści jest dla mnie nieprzydatna.
4. Linki prowadzą do szczegółowych informacji.
5. Potrafię wykorzystać informacje z newslettera.
6. Częstotliwość newslettera pasuje do moich potrzeb.

Pole `powodzenie` zachowuje wspólną nazwę techniczną. Treść pytania: „Czy wykorzystałeś informację z newslettera?”

Źródło wartości: odpowiedź ankietowa — deklaracja respondenta, nie obserwacja wykonania.

Kod 1: zadeklarowano wykorzystanie co najmniej jednej informacji z newslettera w ostatnich czterech tygodniach.
Kod 0: zadeklarowano brak takiego wykorzystania.
NA oznacza nieznany wynik lub brak odpowiedzi, a nie kod 0.

Deklaracja użycia nie jest logiem zachowania ani dowodem skuteczności newslettera; w planie doprecyzuj, jakie działanie uznajesz za wykorzystanie.
