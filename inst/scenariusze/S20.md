# S20 — Portal konferencji naukowej: przejrzystość informacji organizacyjnej

Wersja opisu: 1.1.0. Dane do analizy są syntetyczne.

Organizator konferencji chce ułatwić odnalezienie aktualnych warunków udziału. Porównuje ocenę portalu i rozpoznanie terminu, oddzielając znajomość wydarzenia od cech samej strony.
...
Grupy: pierwszy raz / kolejny raz. Pierwszy raz i kolejny raz oznaczają pierwszy lub kolejny planowany udział w tym cyklu konferencji, ustalony przed zadaniem. Nie są liczbą wizyt na stronie; wcześniejsze uczestnictwo może zmieniać potrzeby informacyjne.

Obserwacyjne badanie przekrojowe; grupy nie są randomizowane.

Indeks: Ocena przejrzystości informacji organizacyjnej. To wynik z sześciu deklarowanych ocen, nie test wszystkich kompetencji ani niezależny audyt usługi.

Pierwsze pytanie: jaka jest różnica średnich indeksu w porównaniu „pierwszy raz minus kolejny raz”? Estymandą jest różnica średnich w populacjach określonych w planie; estymatę obliczamy z dostępnej próby. Jednostka to punkt indeksu. H0 zakłada różnicę równą zero.

Druga relacja: grupa × poprawne odnalezienie terminu.
Drugie pytanie dotyczy różnicy prawdopodobieństw kodu 1: „pierwszy raz minus kolejny raz”. H0 zakłada równe prawdopodobieństwa. Zanim nazwiesz odsetek sukcesem, przeczytaj definicję kodów poniżej.
...
Zadanie: Odnajdź termin zgłoszenia dla wskazanej kategorii udziału i edycji konferencji.
...
1. Łatwo znajduję termin zgłoszenia na konferencję.
2. Portal jasno podaje warunki uczestnictwa.
3. Trudno mi ustalić, która informacja jest aktualna.
4. Program pozwala zaplanować udział.
5. Informacja o opłatach jest zrozumiała.
6. Wiem, gdzie skierować pytanie organizacyjne.

Pole `powodzenie` zachowuje wspólną nazwę techniczną. Treść pytania: „Czy poprawnie odnalazłeś termin zgłoszenia?”

Źródło wartości: zapis obserwatora według podanego kryterium.

Kod 1: podano właściwy termin dla tej kategorii i edycji.
Kod 0: zakończono próbę bez właściwego terminu.
NA oznacza nieznany wynik lub brak odpowiedzi, a nie kod 0.

Poprawność jest sprawdzana według przygotowanego klucza. Znalezienie terminu nie dowodzi zrozumienia wszystkich warunków udziału.
