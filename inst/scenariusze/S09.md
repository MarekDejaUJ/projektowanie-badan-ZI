...
Zespół informacji urzędowej chce wiedzieć, gdzie odbiorcy tracą czas na odnalezienie dokumentu. Ocenia nawigację i czas konkretnej próby, nie wyciągając z nich wniosku o ogólnej jakości urzędu lub kompetencji obywatela.
...
Grupy: często / rzadko. Często i rzadko oznaczają deklarowany utrwalony zwyczaj korzystania z BIP w ostatnim roku. Odrębna kolumna częstotliwości dotyczy ostatnich czterech tygodni; nie wylicza się z niej automatycznie grup.

Obserwacyjne badanie przekrojowe; grupy nie są randomizowane.

Indeks: Ocena łatwości nawigacji w BIP. To wynik z sześciu deklarowanych ocen, nie test wszystkich kompetencji ani niezależny audyt usługi.

Pierwsze pytanie: jaka jest różnica średnich indeksu w porównaniu „często minus rzadko”? Estymandą jest różnica średnich w populacjach określonych w planie; estymatę obliczamy z dostępnej próby. Jednostka to punkt indeksu. H0 zakłada różnicę równą zero.

Druga relacja: czas szukania a indeks.
...
Zadanie: Odnajdź wskazany dokument urzędowy i podaj jego datę oraz miejsce publikacji w BIP.
...
Pole `powodzenie` zachowuje wspólną nazwę techniczną. Treść pytania: „Czy udało się odnaleźć dokument urzędowy?”

Źródło wartości: zapis obserwatora według podanego kryterium.

Kod 1: odnaleziono właściwy dokument.
Kod 0: zakończono próbę bez właściwego dokumentu.
NA oznacza nieznany wynik lub brak odpowiedzi, a nie kod 0.

Szybkie zakończenie próby nie oznacza odczytania dokumentu ze zrozumieniem ani poprawnej oceny jego skutków.
