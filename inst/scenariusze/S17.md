# S17 — Opisy danych w katalogu otwartych danych: czytelność dla odbiorcy

Wersja opisu: 1.1.0. Dane do analizy są syntetyczne.

Zespół katalogu otwartych danych chce ułatwić rozpoznanie, do jakich pytań nadaje się zbiór. Porównuje czytelność z perspektywy odbiorcy z poprawnym rozpoznaniem zakresu; łatwy opis może nadal prowadzić do błędnego zastosowania.
...
Grupy: zawodowi / niezawodowi. Zawodowi: korzystający z katalogu na potrzeby pracy; niezawodowi: korzystający do innego celu podczas tej wizyty. Cel wizyty nie jest poziomem umiejętności ani kategorią zatrudnienia.

Obserwacyjne badanie przekrojowe; grupy nie są randomizowane.

Indeks: Ocena czytelności opisów danych. To wynik z sześciu deklarowanych ocen, nie test wszystkich kompetencji ani niezależny audyt usługi.

Pierwsze pytanie: jaka jest różnica średnich indeksu w porównaniu „zawodowi minus niezawodowi”? Estymandą jest różnica średnich w populacjach określonych w planie; estymatę obliczamy z dostępnej próby. Jednostka to punkt indeksu. H0 zakłada różnicę równą zero.

Druga relacja: grupa × poprawne rozpoznanie zakresu zbioru.
Drugie pytanie dotyczy różnicy prawdopodobieństw kodu 1: „zawodowi minus niezawodowi”. H0 zakłada równe prawdopodobieństwa. Zanim nazwiesz odsetek sukcesem, przeczytaj definicję kodów poniżej.
...
Zadanie: Na podstawie opisu wskaż zakres tematyczny, terytorialny i czasowy podanego zbioru.
...
1. Tytuł zbioru wskazuje, czego dotyczą dane.
2. Opis jasno podaje zasięg terytorialny.
3. Nie rozumiem jednostek użytych w opisie zbioru.
4. Opis pozwala ustalić okres objęty danymi.
5. Definicje zmiennych są zrozumiałe.
6. Potrafię rozpoznać ograniczenia zastosowania zbioru.

Pole `powodzenie` zachowuje wspólną nazwę techniczną. Treść pytania: „Czy poprawnie rozpoznałeś zakres zbioru?”

Źródło wartości: zapis obserwatora według podanego kryterium.

Kod 1: poprawnie wskazano wszystkie trzy elementy zakresu według klucza.
Kod 0: zakończono odpowiedź bez poprawnego wskazania całego zakresu.
NA oznacza nieznany wynik lub brak odpowiedzi, a nie kod 0.

Mimo osobowej formy pytania poprawność koduje obserwator według klucza, nie samoocena respondenta. Jedno zadanie nie potwierdza poprawnego użycia danych.
